...
def pi2(
    fan_speed: Iterable[float],
    step=0.1,
    wattage=20,
    fan_efficacy=1,
    passive_efficacy=0.2,
    thermal_mass=20,
):
    temperature = ROOM_TEMPERATURE
    energy = 0
    ...
    for fan_percent in fan_speed:
        ...
        delta_e = (
            (temperature - ROOM_TEMPERATURE)
            * (fan_percent * fan_efficacy + passive_efficacy)
            * step
        )
        assert delta_e >= 0
...
def algorithm2(pi, step=0.1, setpoint=65, multiplier=1.5):
    """requires tuning"""
    fan_speed = 0.9
    yield fan_speed
    next(pi)
...
def algorithm3(pi, step=0.1, lerp_range=((20, 0), (65, 1))):
    """settles, i think"""
    fan_speed = 0.9
    yield fan_speed
    next(pi)
...
def algorithm4(pi, step=0.1, mintemp=20, maxtemp=65, curvature=0.7):
    """also settles"""
    assert 0 < curvature
    fan_speed = 0.9
    yield fan_speed
...
def algorithm5(pi, step=0.1, single_speed=0.5):
    yield single_speed
    for temperature in pi:
        yield single_speed
...
def print_sim(pi, algorithm, alg_args={}, steps=99999):
    for i, next_temperature, next_speed in run_sim(
        pi, algorithm, alg_args=alg_args, steps=steps
    ):
